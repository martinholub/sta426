...
* I am unclear on `include also the biological factor, knockout state`, but from the previous I understand it as refining the groups by including information on whether the data was acquired as SE or PE. *
...
* One thing I struggle with is defining the design matrix, especially with interaction term. I wouldn't know how to represent it in tems of an equation. *
...
dxd <- DEXSeq::DEXSeqDataSetFromHTSeq(
            ...
            sampleData = samplesX,
            design = ~ exon + cond:exon + type:exon,
            flattenedfile = anno)
...
dxd <- DEXSeq::testForDEU(dxd, reducedModel = ~ cond + exon)
```
